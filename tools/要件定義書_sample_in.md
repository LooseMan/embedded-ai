<!-- 以下は https://notepm.jp/template/requirement-definition よりお借りした例文 -->
...
1. システム開発部門が売り上げデータをCSV出力して、メールで送信
1. 経理担当者がCSVをExcelで開いて編集
1. 経理担当者が経理システムへログイン
1. 経理システムのインポート機能でCSVデータを取り込み
### 構築後のフロー
...
1. バッチシステムが抽出したデータを経理システムへ取り込めるように加工
1. バッチシステムが経理システムへ接続
1. バッチシステムが経理システムへデータを送出
1. バッチシステムがログをログテーブルに記述
1. バッチシステムが経理担当者へバッチ処理完了のメールを送信
1. バッチシステムが対象の売り上げマスターデータへインポート完了のフラグ立て
## 非機能要件
